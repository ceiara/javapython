...
plt.imshow(train_input[0].reshape(28, 28), cmap='gray_r') # 0~255 data
plt.show()

plt.imshow(train_scaled[0].reshape(28, 28), cmap='gray_r') # 0~1 data
plt.show()

# 0티셔츠
pred = sgd.predict(train_scaled[0].reshape(-1, 784)) # 2차원 데이터로 변환
print(pred)
pred = sgd.predict(train_scaled[1].reshape(-1, 784)) # 2차원 데이터로 변환
print(pred)
