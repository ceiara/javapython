...
//        FreeBoard freeBoard = freeBoardRepository.findById(id).orElse(new FreeBoard());
    ...
    public String write(FreeBoardReq freeBoardReq,@RequestParam(required = false) String id){
        ...
        /*
            id 값이 null이면 글쓰기로 들어가 insert 해야함
            id 값이 숫자로 있으면 수정으로 들어가 update 해야 함.
         */
       try {
           if (id != null && !id.equals("")) {
               FreeBoard freeBoard = freeBoardRepository.findById(Long.parseLong(id)).orElse(new FreeBoard());
